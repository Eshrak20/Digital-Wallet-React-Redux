...
export function AppSidebar({ ...props }: React.ComponentProps<typeof Sidebar>) {
  ...
  return (
    <Sidebar 
      {...props} 
      className={`transition-all duration-300 ${darkMode ? 'dark bg-gray-900' : 'bg-gradient-to-b from-blue-50 to-indigo-50'} ${collapsed ? 'w-20' : 'w-64'}`}
    >
          ...
          <SidebarGroup key={group.title} className="border-none">
            {!collapsed && (
              ...
              >
                ...
                <FiChevronDown 
                  className={`transform transition-transform ${expandedGroups[group.title] ? 'rotate-0' : '-rotate-90'}`} 
                />
              </SidebarGroupLabel>
            )}
            
            <SidebarGroupContent className={expandedGroups[group.title] || collapsed ? '' : 'hidden'}>
              <SidebarMenu>
                {group.items.map((item) => (
                  <SidebarMenuItem key={item.title}>
                    ...
                    >
                      <Link to={item.url} className="flex items-center px-3 py-3">
